...
namespace Vision
{
	public class CalibrationPoint
	{
		...
		private float CalculateUnitsPerPixelX(CameraVsPhysicalPoint left, CameraVsPhysicalPoint right)
		{
			...
			{
				pointsUsed += 1;
				unitsPerPixelX += CalculateUnitsPerPixel(left);
			}
			if (right != null)
			{
				pointsUsed += 1;
				unitsPerPixelX += CalculateUnitsPerPixel(right);
			}

			return unitsPerPixelX / pointsUsed;
		}

		private float CalculateUnitsPerPixel(CameraVsPhysicalPoint neighbor)
		{
		...
		private float CalculateUnitsPerPixelY(CameraVsPhysicalPoint above, CameraVsPhysicalPoint below)
		{
			...
			{
				pointsUsed += 1;
				unitsPerPixelY += CalculateUnitsPerPixel(above);
			}
			if (below != null)
			{
				pointsUsed += 1;
				unitsPerPixelY += CalculateUnitsPerPixel(below);
			}
		...
		{
			return new PointF(
				this.PhysicalPoint.X + (cameraPoint.Y - this.CameraPoint.Y) * UnitsPerPixelX,
				this.PhysicalPoint.Y + (cameraPoint.X - this.CameraPoint.X) * UnitsPerPixelY);
		}
